...
GPS1_enlem = 40.809632 # todo: SCRİPT ÇALIŞTIRILMADAN ÖNCE DÜZENLENİR.
GPS1_boylam = 29.264113 #video için: dikdörtgenin 1.köşe koordinatları girilir

GPS2_enlem = 40.809602
GPS2_boylam = 29.264075 #video için: dikdörtgenin 2.köşe koordinatları girilir

GPS3_enlem = 40.809628
GPS3_boylam = 29.263977 #video için: dikdörtgenin 3.köşe koordinatları girilir

GPS4_enlem = 40.809688
GPS4_boylam = 29.263951 #video için: dikdörtgenin 4.köşe koordinatları girilir

GPS5_enlem = 40.809631
GPS5_boylam = 29.264033 #video için: İDA'nın eve dönüş,rıhtım, koordinatları girilir
...
init(autoreset=True)  # Otomatik sıfırlama, stil taşmasını engeller
def log_status(zed,current_x,current_y,adviced_course,hedefe_mesafe,current_goal,manual_mode,magnetic_heading,magnetometer_data,linear_acceleration):
    heading_fark = abs(magnetic_heading - adviced_course) % 360
    if heading_fark > 180:
        heading_fark = 360 - heading_fark

    print(Back.BLACK + Fore.WHITE + Style.DIM + "========== İDA DURUMU ==========" + Style.RESET_ALL)
    print(Back.BLACK + Fore.CYAN + Style.DIM + f"ZAMAN: {datetime.datetime.now().strftime('%H:%M:%S')}")
    
    print(Back.BLACK + Fore.MAGENTA + Style.DIM + f"FPS: {int(zed.get_current_fps())}")
    
    print(Back.BLACK + Fore.YELLOW + f"SOL İTİCİ İTKİ İSTEĞİ (PWM): {controller.get_servo_pwm(SOL_MOTOR)}")
    print(Back.BLACK + Fore.YELLOW + f"SAĞ İTİCİ İTKİ İSTEĞİ (PWM): {controller.get_servo_pwm(SAG_MOTOR)}")

    print(Back.BLACK + Fore.CYAN + Style.BRIGHT + f"İDA GERÇEK HIZ: {controller.get_horizontal_speed()} m/s")


    print(Back.BLACK + Fore.GREEN + f"GERÇEK HEADING: {int(magnetic_heading)}°")
    print(Back.BLACK + Fore.GREEN + f"HEDEF HEADING: {int(adviced_course)}°")
    print(Back.BLACK + Fore.GREEN + Style.DIM + f"HEADING HATASI: {heading_fark:.0f}°")
    print(Back.BLACK + Fore.GREEN + Style.DIM + f"HEADING SAĞLIĞI: {magnetometer_data.magnetic_heading_state}")
    print(Back.BLACK + Fore.GREEN + Style.DIM + f"DOĞRULUK ORANI: {magnetometer_data.magnetic_heading_accuracy:.1f}")

    print(Back.BLACK + Fore.LIGHTRED_EX + f"GÖREV NOKTASI: {current_goal}")
    
    print(Back.BLACK + Fore.CYAN + Style.DIM + f"KONUM: {controller.get_current_position()}")
    print(Back.BLACK + Fore.CYAN + Style.DIM + f"KALAN MESAFE: {int(hedefe_mesafe)} m")
   
    print(Back.BLACK + Fore.LIGHTRED_EX + Style.DIM + f"MANUEL MOD: {manual_mode}")
    print(Back.BLACK + Fore.WHITE + Style.DIM + "================================" + Style.RESET_ALL)
...
def input_thread():
    while True:
        ...
        if c:
            char_queue.put(c)
        time.sleep(0.001)  # CPU tüketimini azaltmak için azıcık uyut
...
def main():
    ...
    # Gri tonlarda boş bir tuval oluştur (tek kanal)
    # 1024x1024 boyutlarında, gri (128,128,128) renkli 3 kanallı tuval
    # tuval = np.full((1800, 1024, 3), (128, 128, 128), dtype=np.uint8) 
    # bu satırlara gerek kalmadı artık veriler terminale işleniyor.
    ...
    while True:
        ...
        key31 = None #klavye okuma için input değişkeni
        try:
            # Kuyruktan hemen bir karakter almaya çalışıyoruz, yoksa None
            key31 = char_queue.get_nowait()
        except queue.Empty:
             pass
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            if zed.get_sensors_data(sensors_data,
                ...
                if ts_handler.is_new(sensors_data.get_imu_data()):
                    ...
                    linear_acceleration = sensors_data.get_imu_data().get_linear_acceleration()[0]
            ...
            if current_goal == GPS1 and not manual_mode:
                ...
                cv2.putText(frame, f"GPS1 e gidiliyor", (10, 420), FONT, 1, (0, 255, 0), 2)
            ...
            elif current_goal == GPS2 and not manual_mode:
                adviced_course = calculate_bearing(ida_enlem, ida_boylam, GPS2_enlem, GPS2_boylam)
                error = signed_angle_difference(magnetic_heading, adviced_course)
                cv2.putText(frame, f"GPS2 e gidiliyor", (10, 420), FONT, 1, (0, 255, 0), 2)
            ...
            elif current_goal == GPS3 and not manual_mode:
                adviced_course = calculate_bearing(ida_enlem, ida_boylam, GPS3_enlem, GPS3_boylam)
                error = signed_angle_difference(magnetic_heading, adviced_course)
                cv2.putText(frame, f"GPS3 e gidiliyor", (10, 420), FONT, 1, (0, 255, 0), 2)
            ...
            elif current_goal == GPS4 and not manual_mode:
                adviced_course = calculate_bearing(ida_enlem, ida_boylam, GPS4_enlem, GPS4_boylam)
                error = signed_angle_difference(magnetic_heading, adviced_course)
                cv2.putText(frame, f"GPS4 e gidiliyor", (10, 420), FONT, 1, (0, 255, 0), 2)
            ...
            elif current_goal == GPS5 and not manual_mode:
                adviced_course = calculate_bearing(ida_enlem, ida_boylam, GPS5_enlem, GPS5_boylam)
                error = signed_angle_difference(magnetic_heading, adviced_course)
                cv2.putText(frame, f"GPS5 e gidiliyor", (10, 420), FONT, 1, (0, 255, 0), 2)
                ...
                cv2.putText(frame, f"EV", (10, 420), FONT, 1, (0, 255, 0), 2)


            # Tuş kontrolü: 'm' tuşu ile modlar arasında geçiş yapılır.
            key = cv2.waitKey(1) & 0xFF
            #todo: çalışmazsa sil key31 = get_char()

            if key == ord('m') or key31 =='m':
            ...
            cv2.putText(frame, f"SOL ITICI HIZ ISTEGI: {(controller.get_servo_pwm(SOL_MOTOR))}", (10, 230), FONT, 1, (0, 255, 0), 2)
            cv2.putText(frame, f"SAG ITICI HIZ ISTEGI: {(controller.get_servo_pwm(SAG_MOTOR))}", (10, 260), FONT, 1, (0, 255, 0), 2)
            cv2.putText(frame, f"IDA GERCEK HIZ: {(controller.get_servo_pwm(SAG_MOTOR))} m/s", (10, 290), FONT, 1, (0, 255, 0), 2)

            cv2.putText(frame, f"IDA GERCEK HEADING: {int(magnetic_heading)}", (10, 320), FONT, 1, (0, 255, 0), 2)
            cv2.putText(frame, f"HEADING ISTEGI: {int(adviced_course)}", (10, 340), FONT, 1, (0, 255, 0), 2)

            cv2.putText(frame, f"KALAN MESAFE: {int(hedefe_mesafe)} metre", (10, 370), FONT, 1, (0, 255, 0), 2)

            cv2.putText(frame, f"KONUM: {(controller.get_current_position())} metre", (10, 390), FONT, 0.5, (0, 255, 0), 2)
            ...
            if key31 == 'q': #terminalde q tuşuna basıldığında motorlar durdurulur ve script kapanır.
                print("Çıkış komutu alındı. Program sonlandırılıyor.")
                #MOCKED: controller.set_servo(SOL_MOTOR,1500)
                #MOCKED: controller.set_servo(SAG_MOTOR,1500)
                break

            clear_terminal()#terminal arayüzü döngüsü sil ve bas komutları.
            log_status(zed,current_x,current_y,adviced_course,hedefe_mesafe,current_goal,manual_mode,magnetic_heading,magnetometer_data,linear_acceleration)
