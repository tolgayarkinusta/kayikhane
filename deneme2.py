...
def log_status(zed,current_x,current_y,adviced_course,hedefe_mesafe,current_goal,manual_mode):
    print("========== IDA DURUMU ==========")
    print(f"FPS: {int(zed.get_current_fps())}")
    print(f"Spatial Mapping State: {zed.get_spatial_mapping_state()}")
    print(f"Coordinates X,Y: {current_x:.1f}, {current_y:.1f}")
    print(f"SOL ITICI PWM: {controller.get_servo_pwm(SOL_MOTOR)}")
    print(f"SAG ITICI PWM: {controller.get_servo_pwm(SAG_MOTOR)}")
    print(f"IDA GERÇEK HEADING: {int(magnetic_heading)}°")
    print(f"HEADING ISTEGI: {int(adviced_course)}°")
    print(f"KALAN MESAFE: {int(hedefe_mesafe)} m")
    print(f"KONUM: {controller.get_current_position()}")
    print(f"GÖREV: {current_goal} konumuna intikal...")
...
def main():
    ...
    while True:
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            log_status(zed,current_x,current_y,adviced_course,hedefe_mesafe,current_goal,manual_mode)
